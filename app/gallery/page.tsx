  ...
  return (
    <div className="agri-page space-y-16 pb-24 pt-12">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center space-y-4">
        <div className="inline-flex items-center gap-2 rounded-full bg-radiate/10 px-3.5 py-1 text-xs font-semibold text-radiate border border-radiate/20">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all ${
                activeCategory === cat
                  ? "bg-radiate text-white shadow-md scale-105"
          ...
          {filteredItems.map((item) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 space-y-1 text-white">
                <span className="inline-block rounded bg-radiate px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                  {item.category}
                </span>
                <h3 className="font-bold text-sm line-clamp-1 group-hover:text-gleam transition-colors">
                  {item.title}
                </h3>
                {item.location && (
                  <p className="text-[11px] text-slate-300 flex items-center gap-1">
                    <MapPin className="h-3 w-3 text-gleam" />
          ...
          <div className="relative w-full max-w-4xl rounded-3xl overflow-hidden bg-slate-950 border border-slate-800 shadow-2xl space-y-4">
            ...
            <div className="p-6 bg-slate-950 space-y-2 text-white border-t border-slate-800">
              <div className="flex items-center justify-between">
                <span className="rounded bg-radiate/20 text-gleam border border-herb/30 px-2.5 py-0.5 text-xs font-bold">
                  {activeLightbox.category}
                </span>
                {activeLightbox.date && (
                  <span className="text-xs text-slate-400 flex items-center gap-1 font-mono">
                    <Calendar className="h-3.5 w-3.5 text-gleam" />
              ...
              {activeLightbox.location && (
                <p className="text-xs text-gleam flex items-center gap-1 pt-1 font-medium">
